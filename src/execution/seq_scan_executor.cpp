...
namespace bustub {
	...
	void SeqScanExecutor::Init() {
		...
		try {
			if (exec_ctx_->GetLockManager()->LockTable(GetExecutorContext()->GetTransaction(),
			...
			try {
				...
				if (res) {
					if (plan_->filter_predicate_ != nullptr) {
						auto value = plan_->filter_predicate_->Evaluate(&iter_->GetTuple().second, table_info->schema_);
