	...
	public:
		...
		void PerformChecks(ExecutorContext *exec_ctx) {
			for (const auto &[left_executor, right_executor]: exec_ctx->GetNLJCheckExecutorSet()) {
				...
				std::cout << "casted_right_executor->GetInitCount() " << casted_right_executor->GetInitCount()
						  << std::endl;
				std::cout << "casted_left_executor" << casted_left_executor->GetNextCount() << std::endl;
