...
// helper function to seperate insert
    void InsertHelperSplit(BPlusTree<GenericKey<8>, RID, GenericComparator<8>> *tree, const std::vector<int64_t> &keys,
        ...
        std::cout << "thread" << thread_itr << tree->DrawBPlusTree() << std::endl;
        delete transaction;
    }

// helper function to delete
    void
    DeleteHelper(BPlusTree<GenericKey<8>, RID, GenericComparator<8>> *tree, const std::vector<int64_t> &remove_keys,
                 __attribute__((unused)) uint64_t thread_itr = 0) {
        GenericKey<8> index_key;
        // create transaction
        auto *transaction = new Transaction(0);
...
// helper function to seperate delete
    ...
    TEST(BPlusTreeConcurrentTest, InsertTest2) {
        ...
        std::cout << "end:" << tree.DrawBPlusTree() << std::endl;
        std::vector<RID> rids;
        GenericKey<8> index_key;
        for (auto key: keys) {
            rids.clear();
