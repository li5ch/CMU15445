...
namespace bustub {
    ...
    bool BPlusTreeLockBenchmarkCall(size_t num_threads, int leaf_node_size, bool with_global_mutex) {
        ...
        // create b+ tree
        BPlusTree<GenericKey<8>, RID, GenericComparator<8>> tree("foo_pk", page_id, bpm, comparator, leaf_node_size,
                                                                 10);
...
//                std::cout << "thread" << i << tree.DrawBPlusTree() << std::endl;
                delete transaction;
            };
...
//		LOG_INFO("success");
    ...
    TEST(BPlusTreeContentionTest, BPlusTreeContentionBenchmark) {  // NOLINT
        std::cout << "This test will see how your B+ tree performance differs with and without contention."
                  << std::endl;
        std::cout
            << "If your submission timeout, segfault, or didn't implement lock crabbing, we will manually deduct all "
